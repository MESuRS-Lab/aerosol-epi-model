...
write.csv2(admission, file = file.path(wd, "..", "data-simplified", "admission.csv"), sep = ";", row.names = FALSE)

agenda <- filter(toy_agenda, ward == chosen_ward)
write.csv2(agenda, file = file.path(wd, "..", "data-simplified", "agenda.csv"), sep = ";", row.names = FALSE)

mat_ctc <- filter(toy_mat_ctc, from %in% admission$id & to %in% admission$id) ## Both induviduals must be from the same ward
write.csv2(mat_ctc, file = file.path(wd, "..", "data-simplified", "mat_ctc.csv"), sep = ";", row.names = FALSE)
```
...
dir.create(file.path(wd,"..", "data-simplified"))
...
for (i in seq_along(list_graph)){
  ...
  write.csv2(df, file = file.path(wd, "..", "data-simplified", paste0("graph_", index, ".csv")), sep = ";", row.names = FALSE)
}
