...
global_status_test[[1]] <- global_status[[1]] %>% mutate(status = ifelse(id == "001-0128-Z-K", as.integer(1), status))
...
test <- simulation(global_interaction = global_interaction,
           ...
           global_status = global_status_test,
...
for (t in 3:3){
  ...
  test_status <- status %>% mutate(status = 0)
  test_lambda <- lambda %>% mutate(lambda_e = 1000, lambda_c = 1000)
  ...
  b = Lambda_c(lambda_tm1, int, status, beta, deltat)
  lambda$lambda_c <- b
  c = Lambda_e(lambda_tm1,  loc, env, admission_test, epsilon, deltat)
  lambda$lambda_e <- c
  d = Get_t(global_localization,t)
  e = Update_environment(env_tm1, loc_tm1, status_tm1, admission_test, mu, nu, deltat )
  env$env <- e
  setdiff(d,global_localization[[t]])
  f = Update_status(test_status, test_lambda)
}
tb = Sys.time()
print(tb-ta)
