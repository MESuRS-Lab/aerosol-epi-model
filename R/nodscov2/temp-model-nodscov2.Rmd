...
test_loc <- data.frame(id = "ba", localization = 1)
test_room <- data.frame(id = c("ab","ba","cd"), id_room = c(1,2,3))
test_env <- test_room %>% select(id_room) %>% mutate(env = 0, id = id_room) %>% select(id, env)
beta = 1
dt = 30
ti = 1
a <- List_encountered("ab", test_int)
b = Lambda_c(test_lambda, test_int, test_status, beta, dt, ti)
test_info_patient_HCW = c(0,1,0)
