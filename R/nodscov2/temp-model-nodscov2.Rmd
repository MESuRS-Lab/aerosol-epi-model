...
## INFO_PATIENT_HCW
```{r}
info_patient_HCW <- admission %>%
  mutate(info = ifelse(status == "PA", 0, 1) ) %>%
  mutate(room = NA) %>% 
  select(id,info,room)
```
...
a <- List_encountered(id, global_interaction[[t]])

test_status <- data.frame(id = c("ab","ba","cd"), status = c(0,1,1))
test_lambda <- data.frame(id = c("ab","ba","cd"), lambda_c = 0, lambda_e = 0)
test_int <- data.frame(from = c("ab","ba","cd"), to = c("ba","ab","ab"))
test_loc <- data.frame(id = c("ab","ba","cd"), localization = c(1,2,3))
test_room <- data.frame(id = c("ba","cd"), id_room = c(2,3))
test_env <- test_room %>% select(id_room) %>% mutate(env = c(200,300), id = id_room) %>% select(id, env)
beta = 1
dt = 30
ti = 1
a <- List_encountered("ab", test_int)
b = Lambda_c(test_lambda, test_int, test_status, beta, dt)
test_info_patient_HCW = data.frame(id = c("ab","ba","cd"), info = c(0,1,1)) # EQUALS TO ADMISSIOn


epsilon = 1

c = Lambda_e(test_lambda,  test_loc, test_env, test_room, test_info_patient_HCW, epsilon, dt)

d = Get_t(global_localization,1)
setdiff(d,global_localization[[1]])
```
