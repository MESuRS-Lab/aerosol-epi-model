...
# Investigation staff are excluded as they were part of the staff deployin gthe sensors
...
length(unique(admission_nodscov2$id)) - length(unique(sensor_nodscovd2$id))
...
p2 = list_ward %>%
  ...
  mutate(cat_from = case_when(cat_from %in% c("Paramedical", "Medical") ~ "HCW",
         ...
         cat_to = case_when(cat_to %in% c("Paramedical", "Medical") ~ "HCW",
                              ...
                              .default = cat_to),
         date_posix_first = strptime(date_posix_first, "%Y-%m-%dT%H:%M:%SZ")
         ) %>%
