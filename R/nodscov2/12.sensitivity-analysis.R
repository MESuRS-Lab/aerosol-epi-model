...
## Plot parameters of epidemic dynamics for all Schemes-----------------------
# Global SAR
stats_df %>%
  ggplot(., aes(x = Scheme, y = Global, fill = parameter_value)) +
...
for (s in unique(stats_df$sensitivity2)) {
  p = stats_df %>%
    ...
    ggplot(., aes(x = Scheme, y = SAR, fill = parameter_value)) +
    geom_violin() +
...
p1 = stats_df %>%
  ...
  ggplot(., aes(x = Scheme, y = Epidemic_duration, col = parameter_value)) +
...
p2 = stats_df %>%
  ...
  ggplot(., aes(x = Scheme, y = Time_to_peak, col = parameter_value)) +
  geom_violin(position = position_dodge(width = 0.7), width = 0.7) +
  geom_boxplot(position = position_dodge(width = 0.7), width = 0.05, outliers = F) +
  facet_grid(cols = vars(network), rows = vars(sensitivity2)) +
  scale_color_manual(values = sensitivity_pal) +
...
p3 = stats_df %>%
  group_by(model, threshold, network, Scheme, sensitivity2, parameter_value) %>%
  summarise(p = sum(Global == 0) / n(), .groups = "drop") %>%
  ggplot(., aes(x = Scheme, y = p, fill = parameter_value)) +
...
bind_rows(
  ...
  group_by(network, Scheme, parameter_value) %>%
  wilcox_test(I_global ~ intervention, ref.group = "None", paired = F, alternative = "two.sided", detailed = T) %>% 
  left_join(., median_none_main %>% filter(Category == "I_global"), by = c("network", "Scheme")) %>%
  mutate(
  ...
  nest(.by = c(network, Scheme, parameter_value)) %>%
  mutate(out = map(data, rank_interventions)) %>%
  select(-data) %>%
  unnest(out) %>%
  ggplot(., aes(x = Scheme, y = group2, fill = estimate)) +
  geom_tile() +
...
ggsave("fig/paper/figure6.eps", height = 4, width = 7)
