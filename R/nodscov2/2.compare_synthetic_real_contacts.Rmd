...
for (net in networks) {
  ...
  for (f in list.files(file.path(synthetic_path, "full", net), pattern = "matContact", full.names = T)) {
...
Analyses are based on the codes from Leclerc et al. (2024) that are available [here](https://gitlab.pasteur.fr/qleclerc/network_algorithm/-/tree/main/analysis?ref_type=heads).
...
for (net in networks) {
  ...
  for (f in list.files(file.path(synthetic_path, "full", net), "matContactBuilt", full.names = T)) {
...
for (net in networks) {
  ...
  # Synthetic networks
  simu_files = list.files(path = file.path(synthetic_path, "full", net), pattern = "matContact.*", full.names = T)
...
ggsave(file.path(fig_path, "metrics.png"), p, height = 5, width = 10)
```
