...
## TEST
```{r}
id_sim <- "new-dev-test-endless-day"
id_sim_path <- file.path(sim_nodscov2_path, id_sim)
fig_sim_path <- file.path(id_sim_path, paste0(id_sim,"-fig"))
...
#load(file.path(id_sim_path, paste0(id_sim, "-", "simulation-nodscov2.RData")))
load(file.path(id_sim_path, paste0(id_sim, "-", "dev-simulation-nodscov2.RData")))
load(file.path(id_sim_path, paste0(id_sim, "-", "new-dev-simulation-nodscov2.RData")))
...
admission_truncated <- admission %>% filter(id %in% id_interacting)
...
print(SEIR_n)
ggsave(file = file.path(fig_sim_path, paste0(id_sim, "-SEIR-n.png")), plot = SEIR_n, height = 4, width = 20)
...
SEIR_p <- ggplot(counts_df, aes(x = time * 30 + begin_date, y = proportion, color = factor(status))) +
  ...
  theme(axis.text.x=element_text(angle=45, hjust=1))
print(SEIR_p)
ggsave(file = file.path(fig_sim_path, paste0(id_sim, "-SEIR-p.png")), plot = SEIR_p, height = 4, width = 20)
```
...
VL_all <- ggplot(viral_load_over_time, aes(x = time, y = viral_load, color = factor(room))) +
  geom_line() +
  ...
  theme(axis.text.x=element_text(angle=45, hjust=1))
print(VL_all)
ggsave(file = file.path(fig_sim_path, paste0(id_sim, "-Viral-Load-ALL.png")), plot = VL_all, height = 4, width = 20)


VL_no_room <- ggplot(viral_load_over_time %>% filter(!room %in% 1:17), aes(x = time, y = viral_load, color = factor(room))) +
  geom_line() +
  labs(title = "Viral Load in Rooms Over Time",
       x = "Time",
       y = "Viral Load",
  ...
  theme(axis.text.x=element_text(angle=45, hjust=1))
print(VL_no_room)
ggsave(file = file.path(fig_sim_path, paste0(id_sim, "-Viral-Load-NO-ROOMS.png")), plot = VL_no_room, height = 4, width = 20)
```
...
LAMBDA_p <- ggplot(FOI_over_time, aes(x = time)) +
  ...
  theme(axis.text.x=element_text(angle=45, hjust=1))
print(LAMBDA_p)
ggsave(file = file.path(fig_sim_path, paste0(id_sim, "-LAMBDA.png")), plot = LAMBDA_p, height = 4, width = 20)
...
FOI_p <- ggplot(FOI_over_time, aes(x = time)) +
  ...
  theme(axis.text.x=element_text(angle=45, hjust=1))
print(FOI_p)
ggsave(file = file.path(fig_sim_path, paste0(id_sim, "-FOI.png")), plot = FOI_p, height = 4, width = 20)
...
visSave(sim_network, file = file.path(fig_sim_path, paste0(id_sim, "-network.html")), selfcontained = T)
```
