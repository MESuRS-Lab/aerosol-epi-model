...
print(SEIR_n)
ggsave(file = file.path(fig_sim_path, 'SEIR', paste0(id_sim, "-SEIR-n.png")), plot = SEIR_n, height = 3, width = 8)
...
## Arbitrary day -> 2020/05/20
arbitrary_day <- date('2020-05-20')

for (id_room in distinct_rooms){
  VL_room <- ggplot(viral_load_over_time %>%
                      filter(room == id_room) %>%
                      filter(date(time) == arbitrary_day),
                    aes(x = time, y = viral_load, color = factor(room))) +
    geom_line() +
    labs(title = paste0("Viral Load in Rooms Over Time - ", id_room),
         x = "Time",
         y = "Viral Load",
