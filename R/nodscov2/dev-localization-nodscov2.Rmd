...
## Check multiple patients in clusters
n_nplet <- do.call(rbind, cluster_info_list) %>%
  group_by(cluster, time) %>%
  summarise(
    n_patients = n(),
    patients = str_c(Patients, collapse = ", "),
    .groups = 'drop'
  ) %>%
  select(time, cluster, patients, n_patients)
```
...
## DIFF BETWEEN DEV AND MAIN BRANCH
```{r}
global_localization_bis <- global_localization
load(file.path(loc_nodscov2_path,"dev-localization-nodscov2.RData")) ##overwrite admission
old <- do.call(rbind, global_localization)
new <- do.call(rbind, global_localization_bis)
