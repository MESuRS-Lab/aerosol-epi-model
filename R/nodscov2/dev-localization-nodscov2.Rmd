...
global_localization <- lapply(1:n_subdivisions, function(t) {
  ...
  lapply(clusters_t, function(cluster) {
    ...
    if (MULTIPLE_patients_in_cluster){
      ...
      cluster_localization <- rooms[rooms$id == "M-PM", "id_room"] ## ONLY MEDICAL AND PARAMEDICAL -> NURSING STATION ?
    } else {
...
```{r}
paths <- do.call(rbind, global_localization) %>% left_join(rooms %>% distinct(id_room, .keep_all = T) %>% select(id_room,room), by = c("localization" = "id_room")) %>% mutate(room = ifelse(is.na(room), "-1 NOT HERE", room))

test_df <- do.call(rbind, global_interaction) %>%
  pivot_longer(cols = c(from, to), names_to = "direction", values_to = "individual") %>% 
  select(individual, time, localization)
...
save_individual_path <- function(ind) {
  ...
  ind_cat <- admission[admission$id == ind, "cat"]
  # filename
  file_name <- file.path(fig_path, paste0(ind,"_", ind_cat, "_fig-traj", ".png"))
...
do.call(rbind,global_localization) %>% filter(id %in% id_patient) %>% group_by(id,localization) %>% summarise(n = n())
```
