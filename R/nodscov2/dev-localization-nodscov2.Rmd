...
smoothing_localization <- function(interactions, localizations) {
  smoothed_loc <- data.frame()
  
  # Récupérer toutes les interactions d'un individu
  interactions_long <- interactions %>%
    pivot_longer(cols = c(from, to), names_to = "direction", values_to = "individual") %>%
    arrange(individual, time)
  
  # Appliquer le lissage pour chaque individu
...
smoothed_global_interaction <- smoothing_localization(interactions, localizations)
...
global_localization <- lapply(1:n_subdivisions, function(t) {
  ...
  for (ind in id_patient){
    localization_t[localization_t$id == ind, localization_t, "localization"] <- rooms[rooms$id == ind, "id_room"]
  }
  return(localization_t)
})
```
