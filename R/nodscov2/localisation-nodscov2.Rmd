...
## CREATE INTERACTION CLUSTERS FOR EACH TIME t
clusters <- lapply(split(data_tot, data_tot$time), function(data) {
...
for (t in 1:n_subdivisions){
  a <- data_tot %>% filter(time == t)
  for (cluster_i in clusters[[t]]){
    for (individual in cluster_i){
      
    }
  }
}

df <- data_tot %>% filter(time == 500)
