...
HCW_interacting_ids <- infered_admission %>% pull(id)
...
global_localisation <- list()


global_localisation <- lapply(1:n_subdivisions, function(t) {
  localisation_t <- data.frame(id = HCW_interacting_ids, localisation = NA)
  ...
  lapply(clusters_t, function(cluster) {
    ...
    if (MULTIPLE_patients_in_cluster){
      ...
      cluster_localisation <- unique(rooms[rooms$id == double_rooms[[1]][[1]], "id_room"])
    } else if (IN_DOUBLE_ROOM_N2) {
      cluster_localisation <- unique(rooms[rooms$id == double_rooms[[2]][[1]], "id_room"])
    } else if (ONE_patient_in_cluster) {
      cluster_localisation <- rooms[rooms$id == cluster[!cluster %in% HCW_interacting_ids], "id_room"]
    } else {
      cluster_localisation <- rooms[rooms$id == "PE", "id_room"] ## ONLY HCW INTERACTING
    }
    ...
    if (MULTIPLE_patients_in_cluster) {
      cluster_possible_localisation <- rooms[rooms$id %in% cluster[!cluster %in% HCW_interacting_ids], "id_room"]
...
global_localisation[[1]]$localisation <- ifelse(is.na(global_localisation[[1]]$localisation), "NOT HERE", global_localisation[[1]]$localisation )
```
...
## SECOND SUBDIVISION
localisation_t1 <- global_localisation[[1]]
localisation_t2 <- global_localisation[[2]]
id_not_interacting_t2 <- localisation_t2 %>% filter(is.na(localisation)) %>% pull(id)
id_room_t1 <- localisation_t1 %>% filter(localisation != rooms[rooms$id == "PE", "id_room"] &
                                           localisation != rooms[rooms$id == "ALL", "id_room"] &
                                           localisation != "NOT HERE" &
                                           !is.na(localisation)
                                         ) %>% pull(id)
  

id_corridor_t1 <- localisation_t1 %>% filter(localisation == rooms[rooms$id == "ALL", "id_room"]) %>% pull(id)
id_restroom_t1 <- localisation_t1 %>% filter(localisation == rooms[rooms$id == "PE", "id_room"]) %>% pull(id)
id_not_here_t1 <- localisation_t1 %>% filter(localisation == "NOT HERE") %>% pull(id)


for (individual in id_not_interacting_t2){
  ## 1. IF INDIVIDUAL IS INTERACTING AT t-1 (localisation_tm1$localisation != "NOT HERE")
  ...
  ## ASSIGN THE LOCALISATION OF THE INDIVIDUAL AT t==2
  localisation_t2[localisation_t2$id == individual, "localisation"] <- ind_loc_t2
}

## CHANGE THE GLOBAL LOCALISATION DATAFRAME AT t==2
global_localisation[[2]]$localisation <- localisation_t2$localisation
...
for (t in 3:n_subdivisions){
  ...
  id_not_interacting_t <- localisation_t %>% filter(is.na(localisation)) %>% pull(id)
  
  id_room_tm1 <- localisation_tm1 %>% filter(localisation != rooms[rooms$id == "PE", "id_room"] &
                                           localisation != rooms[rooms$id == "ALL", "id_room"] &
                                           localisation != "NOT HERE" &
                                           !is.na(localisation)
                                         ) %>% pull(id)
  ...
  id_corridor_tm1 <- localisation_tm1 %>% filter(localisation == rooms[rooms$id == "ALL", "id_room"]) %>% pull(id)
  id_restroom_tm1 <- localisation_tm1 %>% filter(localisation == rooms[rooms$id == "PE", "id_room"]) %>% pull(id)
  id_not_here_tm1 <- localisation_tm1 %>% filter(localisation == "NOT HERE") %>% pull(id)
  ...
  for (individual in id_not_interacting_t){
    ...
    ## ASSIGN THE LOCALISATION OF THE INDIVIDUAL AT t==2
    localisation_t[localisation_t$id == individual, "localisation"] <- ind_loc_t
  }
  
  global_localisation[[t]] <- localisation_t
}
...
## SUBDIVIDING EVERY 30s (dataframe) - OUTDATED (LIST OF DATAFRAMES NOW)
```{r}
begin_date <- as.POSIXct(min(data$date_posix_first))
end_date <- as.POSIXct(max(data$date_posix_first + data$length))
time_spent <- end_date - begin_date
...
## ISSUE WITH LAPPLY (NOT DOING IT IN ORDER, IT MUST BE ITERATIVE)
```{r}

global_localisation <- lapply(1:n_subdivisions, function(t) {
  if (t == 1){ 
    return (global_localisation[[1]])
  } else if (t == 2) {
    return (global_localisation[[2]])
  } else {
