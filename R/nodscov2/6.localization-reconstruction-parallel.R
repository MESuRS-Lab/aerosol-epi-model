...
verif = foreach (network=networks, .combine=c) %dopar% {
  ...
  if (db == "simulated") {
    ...
    # Interaction data
    interactions = read.csv2(file.path(nodscov2_synthetic_path, "full", net, "truncated_data_1.csv")) %>%
  ...
  if (db == "observed") {
    ...
    agenda = read.csv2(file.path(nodscov2_path, "clean", paste0("agenda_cleaned_", net,".csv"))) %>%
      mutate(firstDate = as_datetime(DATEREMISE), lastDate = as_datetime(DATEREC))
  ...
  # if (db == "simulated") write.csv2(interactions, paste0("data/data-synthetic-graphs/input/interactions_", net, "_trimmed.csv"), row.names = F)
