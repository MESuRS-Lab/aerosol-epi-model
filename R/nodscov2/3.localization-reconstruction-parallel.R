...
# Network to study
networks = c("herriot_simulated", "poincare_simulated", "herriot_observed", "poincare_observed")
network = "herriot_simulated"

# registerDoParallel(4)
# verif = foreach (network=networks, .combine=list) %dopar% {

#   
#   checkpoints
# }
# cat(verif,file="out/loc-reconstruction/process-verification.txt")
...
if (db == "simulated") {
  ...
  # Interaction data 
  interactions = read.csv2(file.path(nodscov2_synthetic_path, "full", net, paste0(net, "matContactBuiltSimulatedCtcNetworks1_oneday.csv"))) %>%
...
locations = get_global_location(clusters, admission, n_subdivisions, rooms )
...
# otherwise (Patients)
identical(colnames(locations[, id_patient]), id_patient)
for (i in id_patient) {
  locations[,i] = patient_locations(
    i, 
    locations[, i], 
    admission = admission, 
...
# locations[,id_patient] = as_tibble(mapply(
#   patient_locations, 
#   patient_id = id_patient, 
#   patient_loc = locations[, id_patient],
...
identical(colnames(locations[, id_hcw]), id_hcw)
...
for (i in id_hcw) {
  #locations[,i] = 
    ...
    begin_date = begin_date, 
    end_date = end_date, 
    patient_rooms_ids = patient_rooms_ids, 
