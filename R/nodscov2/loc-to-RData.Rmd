...
beta <- 1 * 24# Transmission rate for close contact interaction /h
nu <- 1 * 24 # (10 quanta/h)
```
...
intervention_path <- file.path(out_path, 'intervention-analysis')
RData_path <-  file.path(intervention_path, "parameters-admission-nodscov2.RData")
...
## Intervention analysis (no replicated truncated data, new patients)
beta = 3/4
nu = 12
...
## SAVE FOR INTERVENTION ANALYSIS SINGLE SIMULATION
save(admission,
     admission_sim,
     beta,
     nu,
     ...
     t_end,
     data_sim,
     file = file.path(intervention_path, "scenario-3-unique-simulation-nodscov2.RData")
     )
