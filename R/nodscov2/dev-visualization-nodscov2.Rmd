...
load(file.path(sensibility_path, '02-sensibility-analysis.RData'))
```
...
plot_SEIR <- function(counts_list, median_counts, min_counts, max_counts, title, fig_sim_path, id_sim) {
  ...
  print(SEIR_n)
  ggsave(file = file.path(fig_sim_path, paste0(id_sim, "-SEIR-n-", title, ".png")), plot = SEIR_n, height = 4, width = 20)
  ...
  SEIR_n_t <- SEIR_n +
    ...
    geom_line(data = max_counts,
              ...
              linetype = "solid")
  print(SEIR_n_t)
  ggsave(file = file.path(fig_sim_path, paste0(id_sim, "-SEIR-n-total", title, ".png")), plot = SEIR_n_t, height = 4, width = 20)
  ...
  print(SEIR_p)
  ggsave(file = file.path(fig_sim_path, paste0(id_sim, "-SEIR-p-", title, ".png")), plot = SEIR_p, height = 4, width = 20)
  ...
  SEIR_p_t <- SEIR_p +
    ...
    geom_line(data = max_counts,
              ...
              linetype = "solid")
  print(SEIR_p_t)
  ggsave(file = file.path(fig_sim_path, paste0(id_sim, "-SEIR-p-", title, ".png")), plot = SEIR_p_t, height = 4, width = 20)
}


plot_SEIR(counts_C, median_counts_C, min_counts_C, max_counts_C, "Contact", fig_sim_path, id_sim)
plot_SEIR(counts_E, median_counts_E, min_counts_E, max_counts_E, "Environment", fig_sim_path, id_sim)
plot_SEIR(counts_C_E, median_counts_C_E, min_counts_C_E, max_counts_C_E, "Contact and Environment", fig_sim_path, id_sim)
```
...
plot_SEIR <- function(counts_list, median_counts, min_counts, max_counts, title, fig_sim_path, id_sim) {
  ...
  SEIR_n <- ggplot() +
    #geom_line(data = counts_df,
              #aes(x = time * 30 + begin_date, y = count, color = status, group = interaction(time, status)),
              #alpha = 0.2) +
    geom_line(data = median_counts,
              aes(x = time * 30 + begin_date, y = count, color = status),
              linewidth = 0.5,
  ...
  SEIR_p <- ggplot() +
    geom_line(data = counts_df, aes(x = time * 30 + begin_date, y = proportion, color = status, group = interaction(time, status)), alpha = 0.2) +
    geom_line(data = median_counts,
              aes(x = time * 30 + begin_date, y = proportion, color = status),
              linewidth = 1.5,
              linetype = "solid") +
