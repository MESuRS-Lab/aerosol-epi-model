...
ggsave("fig/paper/figure2.eps", p_final, height = 8, width = 12)
...
p3 = stats_df %>%
  ...
  # ggboxplot(., x = "Scheme", y = "Global", width = 0.4, outlier.shape = NA, col = "network") +
  # stat_pvalue_manual(p_sar, label = "p.adj.signif") +
  ggplot(., aes(x = Scheme, y = Global, col = network)) +
...
stats_df %>%
  nest(.by = network) %>%
  mutate(kruskall_wallis_p = map_dbl(data, ~kruskal.test(.$Global ~ .$Scheme)$p.value))

# SAR by source
p4 = stats_df %>%
  pivot_longer(c(Contact, Environment), names_to = "Route", values_to = "SAR") %>%
  mutate(Route = ifelse(Route == "Contact", "Short-range", "Long-range")) %>%
  ggplot(., aes(x = Scheme, y = SAR, col = Route, group = interaction(Scheme, Route))) +
...
ggsave("fig/paper/figure3.eps", p_all, height = 7, width = 7)
...
stats_df %>%
  mutate(Extinction = ifelse(Global == 0, "Yes", "No")) %>%
  count(network, Extinction, Scheme) %>%
...
# Kruskall-Wallis test to compare time to the peak across transmission schemes for 
# each transmission network
stats_df %>%
  nest(.by = network) %>%
  mutate(kruskall_wallis_p = map_dbl(data, ~kruskal.test(.$Epidemic_duration ~ .$Scheme)$p.value))
...
stats_df %>%
  nest(.by = network) %>%
  mutate(kruskall_wallis_p = map_dbl(data, ~kruskal.test(.$Time_to_peak ~ .$Scheme)$p.value))
...
ggsave("fig/paper/figure4.eps", p_all, height = 6, width = 10)
...
stats_df %>%
  ...
  ggplot(., aes(x = Scheme, y = SAR, fill = Pair)) +
