...
for (ind in id_individual %>% filter(substr(id,1,2) == "PA") %>% pull(id)){
  ...
  for (adm in seq_len(admission %>% filter(id == ind) %>% nrow())) {
    ...
    assign(paste0("int_",adm), night_ind %>% filter(between(date_posix, date_adm, date_departure)))
    print(paste0("ALL INTERACTIONS : ", ind," -> ",adm, " nb int = ", night_ind %>% filter(between(date_posix, date_adm, date_departure) ) %>% nrow()))
...
for (ind in id_individual %>% filter(substr(id,1,2) == "PA") %>% pull(id)){
  ...
  for (adm in seq_len(admission %>% filter(id == ind) %>% nrow())) {
    ...
    date_departure <- admission %>% filter(id == ind) %>% nth(adm) %>% pull(lastDate)
    assign(paste0("int_",adm), night_ind %>% filter(between(date_posix, date_adm, date_departure)))
    print(paste0("NIGHT TIME : ", ind," -> ",adm, " nb int = ", night_ind %>% filter(between(date_posix, date_adm, date_departure) ) %>% nrow()))
