...
## Data import (i-Bird)
```{r}
adm_data = read.csv(file.path(wd, "..", "..", "data","data-observed", "toy_admission.csv"), sep=";") %>%
...
adm_data = adm_data[,c(1,3,4)] %>%
  distinct()
eq_table = openxlsx::read.xlsx(file.path(wd, "..", "..","data","data-observed", "cat_groupings.xlsx")) %>%
  select(cat, cat_ag)
adm_data = adm_data %>%
  left_join(eq_table, by = "cat") %>%
  mutate(staff = grepl("PE-", id))

data = read.csv2(file.path(wd, "..", "..","data","data-simplified", "graph_1.csv"))
```
...
ggsave(file.path(wd, "..", "..", "Figures", "fig1.png"), height = 12, width = 10)
