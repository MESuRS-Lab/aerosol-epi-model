...
Rcpp::DataFrame simulation(
    ...
    for (size_t i=0; i<inf_by_t0.size(); i++) {
        if (inf_by_t0[i] == "INDEX") {
            index_patient_id = i;
            index_patient = ids_all[i];
        }
    }
    ...
    status_tim1 = Get_status_t_variant(global_status, ids_ti, 0, index_patient); 
    interaction_with_patient_ti = Get_t(global_data, 0)["interaction_with_patient"];

    // Shedding of the index patient
    environment_ti["env"] = Update_environment(
        ids_ti, 
        info_ti, 
        environment_tim1, 
        location_ti, 
        status_tim1, 
        interaction_with_patient_ti, 
        mu, 
        mu_int, 
        nu, 
        nu_int, 
    ...
    int recovery_time_index = t_recover[index_patient_id];

    for (int t = 1; t < recovery_time_index+1; t++){ // 
        ...
        status_tim1 = Get_status_t_variant(global_status, ids_ti, t-1, index_patient);
        status_ti = Get_status_t_variant(global_status, ids_ti, t, index_patient);
        ...
        environment_ti["env"] = Update_environment(
            ids_ti, info_ti, environment_tim1, location_ti, status_tim1, interaction_with_patient_ti, 
